...
nnoremap <silent> gw :call <sid>SwapWithNext('keep', 'w')<cr>:silent! call repeat#set("gw")<cr>
" Swap the current word with the previous, without changing cursor position
nnoremap <silent> gW :call <sid>SwapWithPrev('keep', 'w')<cr>:silent! call repeat#set("gW")<cr>

" Swap the current word with the previous, keeping cursor on current word:
" (This feels like "pushing" the word to the left.)
nnoremap <silent> gh :call <sid>SwapWithPrev('left', 'w')<cr>:silent! call repeat#set("gh")<cr>

" Swap the current with word the next, keeping cursor on current word: (This
" feels like "pushing" the word to the right.) (See note.)
nnoremap <silent> gl :call <sid>SwapWithNext('right', 'w')<cr>:silent! call repeat#set("gl")<cr>

" the same, but with keywords
nnoremap <silent> gs :call <sid>SwapWithNext('follow', 'k')<cr>:silent! call repeat#set("gs")<cr>
nnoremap <silent> gS :call <sid>SwapWithPrev('follow', 'k')<cr>:silent! call repeat#set("gS")<cr>

" While we're talking swapping, here's a map for swapping characters:
nnoremap <silent> gc xph:silent! call repeat#set("gc")<cr>
...
" {cursor_pos} values:
" 'keep' : stays at the same position
" 'follow' : with the stays current word, at the same relative offset
" 'right' : put the cursor at the start of the new right word
" 'left' : put the cursor at the start of the new left word
" todo: move to an autoplugin
